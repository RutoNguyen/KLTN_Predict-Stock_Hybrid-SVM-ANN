...
def process():
    ...
    st.markdown("# Summary ")
    st.markdown("#### Report performance")
    table = {'Name': ["ARIMA" , "LSTM", "ARIMA-LSTM", "ARIMA-LSTM cải tiến"], 
            'Time(s)':[t_ARIMA_Train+ t_ARIMA_Test, t_LSTM_Train+ t_LSTM_Test, t_hybrid_train+t_hybrid_test, t_hybrid2_train+ t_hybrid2_test],
            'RMSE': [rmse, rmse_l, rmse_h_2, rmse_h2], 
            'MAE': [mae, mae_l, mae_h_2, mae_h2]}
    st.table(table)
    #plot
    st.markdown("#### Plot data Predict")
    plot2grap(np.array(test_data[g_column_select]).reshape(-1), np.exp(fc_series), "Real", "ARIMA")
